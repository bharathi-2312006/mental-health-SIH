...
function MyJourney() {
  ...
  // --- This useEffect hook is now more robust ---
  useEffect(() => {
    // onAuthStateChanged is the most reliable way to get the current user, especially after a refresh
    const unsubscribe = auth.onAuthStateChanged(user => {
      if (user) {
        setUserName(user.displayName ? user.displayName.split(' ')[0] : 'My');

        // Fetch screenings for the logged-in user
        const screeningsQuery = query(collection(db, "screenings"), where("userId", "==", user.uid), orderBy("createdAt", "asc"));
        ...
        // Return a cleanup function for the data listeners
        return () => {
          screeningsUnsub();
          journalUnsub();
        };
      } else {
        // No user is logged in, stop loading and clear data
        setLoading(false);
        setScreenings([]);
        setJournalEntries([]);
        setUserName('');
  ...
  return (
    <div className="min-h-screen bg-gray-900 pt-48 sm:pt-56 px-4 sm:px-8 relative text-white">
      <HeartParticleCanvas />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
          <div className="lg:col-span-3 space-y-8">
            <div className="bg-white/10 backdrop-blur-md p-6 rounded-2xl shadow-lg border border-white/20">
              <h2 className="text-2xl font-bold mb-4 flex items-center gap-3"><FaBookMedical className="text-blue-400" /> Screening History</h2>
              <div className="space-y-4 max-h-96 overflow-y-auto pr-2">
                {loading && <p>Loading history...</p>}
                {!loading && screenings.length === 0 && <p className="text-gray-400">You haven't completed any screenings yet.</p>}
                {screenings.map(s => (
                  <div key={s.id} className="border-l-4 p-4 rounded-r-lg bg-black/20" style={{ borderColor: s.result === 'High' ? '#ef4444' : s.result === 'Moderate' ? '#f59e0b' : '#22c55e' }}>
                    <p className="font-semibold text-lg">{s.result} Risk Level (Score: {s.score})</p>
            ...
            <div className="bg-white/10 backdrop-blur-md p-6 rounded-2xl shadow-lg border border-white/20">
              <h2 className="text-2xl font-bold mb-4 flex items-center gap-3"><FaPenFancy className="text-purple-400" /> Private Journal</h2>
              <form onSubmit={handleAddEntry} className="mb-4">
                <textarea value={newEntry} onChange={(e) => setNewEntry(e.target.value)} placeholder="Write down your thoughts..." className="w-full p-3 border rounded-lg h-28 bg-white/10 border-white/20 text-white focus:ring-2 focus:ring-purple-400" />
          ...
          <div className="lg:col-span-2 bg-white/10 backdrop-blur-md p-6 rounded-2xl shadow-lg border border-white/20">
            <h2 className="text-2xl font-bold mb-4 flex items-center gap-3"><FaChartLine className="text-green-400" /> Your Progress</h2>
            {loading ? <p>Loading chart...</p> : screenings.length < 2 ? (
                <div className="flex flex-col items-center justify-center h-full text-center">
                    <p className="text-gray-400">Complete at least two screenings to see your progress chart.</p>
                </div>
            ) : (
